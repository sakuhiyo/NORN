def plasma_dice(pow,int):
    ...
    /* ページ全体のフォントサイズを設定 */
    html, body, [class*="css"] {
        font-size: 14px; /* フォントサイズを調整（例: 16px） */
    }
    </style>
""", unsafe_allow_html=True)
    ...
    st.markdown("""
        ...
        <h3 style="font-size:20px;">\U0001f3b2 プラズマダイス \U0001f3b2</h3>
        </div>
	""", unsafe_allow_html=True)
    ...
    if st.session_state["play_count"] < 4:
        if st.button("プレイする", key="plasma_dice_play_button"):
            ...
            st.markdown(f"""
            <h3 style="font-size:20px;">最終得点</h3>
            <p style="font-size:16px;"><strong>基本得点</strong>: {base_score}点</p>
            <p style="font-size:16px;"><strong>倍率（マルチプライヤー）</strong>: ×{multiplier}</p>
            <p style="font-size:16px;"><strong>プラズマボーナス</strong>: ×{plasma_bonus}</p>
            <p style="font-size:16px;"><strong>計算式</strong>: {base_score} × {multiplier} × {plasma_bonus} → <strong>{final_score}点</strong></p>
            """, unsafe_allow_html=True)
